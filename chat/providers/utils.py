...
def add_language_hint(question: str) -> str:
    ...
    if lang in ("gujarati", "gujarati_roman"):
        ...
        hint = "कृपया हिंदી में उत्तर दें।"
    else:
...
def _build_rules(fallback_contact: str = "") -> str:
    ...
    return (
        "STRICT RULES:\n"
        "1. LANGUAGE — CRITICAL: The user's message ends with a language instruction in parentheses.\n"
        "   You MUST reply in that language only. The document's own language is completely irrelevant.\n"
        "   • Hindi hint (कृपया हिंदी में उत्तर दें) → write ONLY Devanagari. Zero Gujarati script (ગ,ા,ે…) in the reply.\n"
        "   • Gujarati hint (ગુજરાતી ભાષામાં જ જવાબ આપો) → write ONLY Gujarati script. Zero Devanagari (ग,ा,े…) in the reply.\n"
        "   • English hint (Please reply in English) → write ONLY English. No Gujarati or Devanagari script.\n"
...
def build_document_prompt(markdown_text: str, fallback_contact: str = "") -> str:
    ...
    return (
        ...
        "the most closely related information that IS present — but for degree/course names, "
        "NEVER answer about a different degree (BA ≠ BCA ≠ BE; MA ≠ MCA ≠ MBA — all distinct). "
